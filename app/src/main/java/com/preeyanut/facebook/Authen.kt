...
class Authen : Fragment() {
    ...
    private fun handleFacebookAccessToken(token : AccessToken) {
        Log.d("handleFacebookAccessToken:",token.toString())
        ...
        firebaseAuth!!.signInWithCredential(credential).addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                transaction.replace(R.id.main_layout, profile,"fragment_profile")
                transaction.addToBackStack("fragment_profile")
                transaction.commit()

            } else {
                // If sign in fails, display a message to the user.
                Log.d("signInWithCredential:",task.exception.toString())
